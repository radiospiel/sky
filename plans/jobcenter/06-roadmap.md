...
| `interface.rb`, JC HTTP endpoints | `api/` (ConnectRPC) |
| `cli/*` | `cmd/jobcenter` |
...
- **Remote-worker model** — *decided:* runners and clients always connect to the server over a [ConnectRPC](https://connectrpc.com) API (protobuf + JSON); only the server touches the database. No DB-direct workers. The session/host endpoints are therefore part of the core surface, not optional.
